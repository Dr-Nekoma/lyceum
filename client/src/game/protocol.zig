...
pub fn pingUpdateCharacter(gameState: *GameState) !void {
    ...
    const server_response = try messages.receive_characters_list(gameState.allocator, gameState.connection.node);
...
pub fn pingJoinMap(gameState: *GameState) !void {
    ...
    const server_response = try messages.receive_standard_response(gameState.allocator, gameState.connection.node);
    switch (server_response) {
        .ok => {},
        .@"error" => |msg| {
            defer gameState.allocator.free(msg);
