{
  ...
  in (flake-utils.lib.eachDefaultSystem (system: let
    ...
    erlang = "${pkgs.erlang}/lib/erlang/lib/erl_interface-5.3.2.1/";
    raylib = pkgs.raylib;
    # raylib-lib = "${pkgs.raylib}/lib";            
  in with builtins; with env.lib; with env.pkgs.lib; rec {
      ...
      # nix run .
      apps.default = env.app [] "zig build --search-prefix ${erlang} --search-prefix ${raylib} run -- \"$@\"";

      # nix run .#build
      apps.build = env.app [] "zig build --search-prefix ${erlang} --search-prefix ${raylib} \"$@\"";
      
      # nix run .#test
      apps.test = env.app [] "zig build --search-prefix ${erlang} --search-prefix ${raylib} test -- \"$@\"";

      # nix run .#docs
      apps.docs = env.app [] "zig build --search-prefix ${erlang} --search-prefix ${raylib} docs -- \"$@\"";
