...
const LNode = struct {
    c_node: ei.ei_cnode,
    fd: i32,
    node_name: [*c]const u8 = "lyceum_client@nixos",
    port: i32 = 4369,
    identification_number: i32 = 99,
    cookie: [*c]const u8 = "lyceum",
};
...
pub fn prepare_connection() !LNode {
    ...
    const creation_c: c_uint = @truncate(creation_u);
    const result = ei.ei_connect_init(&l_node.c_node,
                                      l_node.node_name,
                                      l_node.cookie,
                                      creation_c);
    if (result < 0) return error.ei_connect_init_failed;
    return l_node;
}
