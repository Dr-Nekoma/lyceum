...
check_user(#{username := Username, password := Password}, Connection) ->
    Query = "SELECT * FROM lyceum.user WHERE username = $1::VARCHAR(32) AND password = $2::TEXT",
...
insert_user(#{username := Username, email := Email, password := Password}, Connection) ->
    Query = "INSERT INTO lyceum.user  (username, e-mail, password) VALUES ($1::VARCHAR(32), $2::TEXT, $3::TEXT)",
