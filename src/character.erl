...
-export([create/2, player_characters/3, update/2, updateTemp/2, retrieve_near_players/3, activate/3, deactivate/2]).
...
update(#{name := Name, 
    ...
    Query = "UPDATE lyceum.view_character SET x_position = $1::SMALLINT, y_position = $2::SMALLINT, x_velocity $7::DECIMAL, y_velocity = $8::DECIMAL \ 
             WHERE name = $3::VARCHAR(18) AND e_mail = $4::TEXT AND username = $5::VARCHAR(32) AND map_name = $6::VARCHAR(64)",
    {ok, _} = epgsql:equery(Connection, Query, [XPosition, YPosition, Name, Email, Username, MapName, XVelocity, YVelocity]).
...
updateTemp(#{name := Name, 
	     ...
	     y_velocity := YVelocity}, Connection) ->
    %% io:format("x: ~p, y: ~p\n", [XPosition, YPosition]),
    Query = "UPDATE lyceum.character_position SET x_position = $1::SMALLINT, y_position = $2::SMALLINT, face_direction = $7::SMALLINT, x_velocity $8::DECIMAL, y_velocity = $9::DECIMAL \ 
             WHERE name = $3::VARCHAR(18) AND e_mail = $4::TEXT AND username = $5::VARCHAR(32) AND map_name = $6::VARCHAR(64)",
