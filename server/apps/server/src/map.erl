...
get_map(MapName, Connection) ->
    ...
    Objects =
        epgsql:equery(Connection,
                      ...
                      [MapName]),
    FunDimensions =
        fun(FullColumns, Values) -> {ok, database_utils:columns_and_rows(FullColumns, Values)}
        end,
    FunTiles =
        fun(FullColumns, Values) ->
           {ok,
            lists:map(fun transform_tile/1, database_utils:columns_and_rows(FullColumns, Values))}
        end,
    FunObjects =
        fun(FullColumns, Values) ->
           {ok,
            lists:map(fun transform_object/1, database_utils:columns_and_rows(FullColumns, Values))}
        end,
    database_utils:psql_bind(
        database_utils:process_postgres_result(Dimensions, select, FunDimensions),
        [fun(ListDimensionsMap) ->
         ...
         fun({Width, Height}) ->
            database_utils:psql_bind(
                database_utils:process_postgres_result(Tiles, select, FunTiles),
                [fun(TilesV) ->
                    database_utils:psql_bind(
                        database_utils:process_postgres_result(Objects, select, FunObjects),
                        [fun(ObjectsV) ->
